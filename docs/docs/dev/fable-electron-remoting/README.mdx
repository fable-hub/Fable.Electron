...
The Main API now provides explicit entry points for this:

* `Remoting.fromValue`: use a direct implementation value and pass only payload args.
* `Remoting.fromIpcMainEvent`: use an `IpcMainEvent -> 'Api` factory so the event context is injected on each call.

This removes implicit reflection-based first-argument detection and keeps shared
API signatures focused on renderer-callable payloads.
