...
:::note Disposable Handlers
For UI lifecycles (for example React unmount), prefer `Remoting.buildProxyReceiverDisposable` on the renderer.
It returns a disposer with signature `unit -> unit` that unsubscribes all registered listeners for that handler registration.

The returned disposer is idempotent, so calling it more than once is safe.
:::
...
let createWindow() =
    ...
    let sendMsg =
        ...
        |> Remoting.buildProxySender<Shared.ExampleMainToRenderer>
...
// Later, when unmounting / tearing down subscriptions:
disposeHandler ()
disposeHandler () // safe: second call is a no-op
// highlight-end
```
</TabItem>
</Tabs>

If you do not need a disposer, use `Remoting.buildProxyReceiver`.
It registers the handler and discards the returned disposal function.

```fsharp
let handler = { Shared.LogMove = fun t i -> console.log t }
Remoting.createIpc ()
|> Remoting.buildProxyReceiver handler
```
