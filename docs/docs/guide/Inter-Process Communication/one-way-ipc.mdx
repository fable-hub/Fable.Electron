...
For UI lifecycles (for example React unmount), prefer `Remoting.buildHandlerDisposable` on the renderer.
...
let disposeHandler =
    Remoting.init
    |> Remoting.buildHandlerDisposable handler
...
For backward compatibility, `Remoting.buildHandler` is still available and behaves the same as before.
It registers the handler and discards the disposer.
