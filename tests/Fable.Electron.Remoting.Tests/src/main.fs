...
app
    ...
    .``then`` (fun () ->
        ...
        let handler = {
            Increment = fun () -> promise {
                setCounter {
                    counter with
                        ClickCount =
                            if counter.Disabled
                            then counter.ClickCount
                            else counter.ClickCount + 1
                        Value =
                            if counter.Disabled
                            then counter.Value
                ...
                if counter.Disabled then return Error()
                else return Ok counter.Value
            }
            Decrement = fun () -> promise {
                setCounter {
                    counter with
                        ClickCount =
                            if counter.Disabled
