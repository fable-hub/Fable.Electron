...
let inline getIncButton() = browser.``$`` "#counter-button-increment"
let inline getDecButton() = browser.``$`` "#counter-button-decrement"
...
describe "App loads correctly" <| fun _ -> promise {
    it "Buttons and label exist" <| fun _ -> promise {
    ...
    describe "Initial state is correct" <| fun _ ->
        it "Label is zero" <| fun _ -> promise {
            let! t = getLabel() |> getInt
            Expect.equal t 0 "Label should be 0"
        }
        it "Enable is disabled" <| fun _ -> promise {
            let! en = getEnableButton()
            let! actual = en.isEnabled()
            Expect.isFalse actual "Enable should be disabled"
        }
        it "Disable is enabled" <| fun _ -> promise {
            let! dis = getDisableButton()
...
describe "Combination TWO Way and ONE Way IPC works" <| fun _ -> promise {
    it "Value responds to inc/dec clicks" <| fun _ -> promise {
        let! label = getLabel()
    ...
    it "Disable and Enable round trip correctly" <| fun _ -> promise {
        let! dis = getDisableButton()
        let! en = getEnableButton()
    ...
    it "Disable prevents inc/dec" <| fun _ -> promise {
        let! dis = getDisableButton()
        let! en = getEnableButton()
        let! label = getLabel()
        do! expect(dis).toBeEnabled()
