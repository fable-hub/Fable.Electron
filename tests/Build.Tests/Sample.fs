...
let specModules =
    Assembly.GetAssembly(typeof<Args>).GetTypes()
    |> Array.filter FSharpType.IsModule

let findModule (name: string) =
    specModules |> Array.find _.Name.StartsWith(name)
...
let commands =
    ...
    |> Array.choose (
        function
        | :? FieldInfo as f when not f.IsSpecialName -> Some(f.Name, f.GetRawConstantValue() :?> string)
        | _ -> None
    )

let commandMap = Map commands

let makeMap (typ: System.Type) =
    if not <| FSharpType.IsModule typ then
        failwith $"makeMap has to be used on modules. {typ.Name} is not a module"
    ...
    testList
        ...
        [ testList
              ...
              [ let projects, delta = Workers.Versions.ElectronDelta.CreateFromContext()
                ...
                let getProject name =
                    try
                        projects |> Project.Cracked.getProjectOrFail name |> Ok
                    with e ->
                        Error e
                ...
                testCase "Loads electron by scope"
                <| fun _ -> getProject "Electron" |> Flip.Expect.isOk "Fable.Electron is found by scope"

                testCase "Loads forge by scope"
                <| fun _ -> getProject "Forge" |> Flip.Expect.isOk "Fable.Electron.Forge is found by scope"

                testCase "Loads remoting by scope"
                <| fun _ ->
                    getProject "Remoting"
                    |> Flip.Expect.isOk "Fable.Electron.Remoting is found by scope"

                testCase "Electron scoped project points to Fable.Electron.fsproj"
                <| fun _ ->
                    getProject "Electron"
                ...
                testCase "Forge scoped project points to Fable.Electron.Forge.fsproj"
                <| fun _ ->
                    getProject "Forge"
                ...
                testCase "Remoting scoped project points to Fable.Electron.Remoting.fsproj"
                <| fun _ ->
                    getProject "Remoting"
                ...
                testList
                    "Delta"
                    [ testCase "Delta Version for Project Props are Some"
                      <| fun _ ->
                          (delta.Versions.FableElectronElectron.IsSome
                           && delta.Versions.FableElectronPackage.IsSome)
                          |> Flip.Expect.isTrue "Project version props yield values for delta"
                      testCase "Cache/Downloaded versions are none"
                      <| fun _ ->
                      ...
                      testCase "Cache version loads as ReleaseInfo"
                      <| fun _ ->
                          Files.Cache
                          |> Fake.IO.File.exists
                          ...
                          Status.tryGetCache () |> Flip.Expect.isSome "Release info loaded into cache"
...
let deltas =
    ...
    let makeDelta downloaded fable =
        ElectronDelta.Create(
            fableElectronElectronProperty = fable,
            downloadedVersion = downloaded,
            fableElectronPackage = fable
        )

    testList
        "Deltas"
        [ testTheory
              "Electron Bumps Versions"
          ...
          testTheory
              ...
              (fun para ->
                  ...
                  |> (if para.isPulled then
                          Flip.Expect.isTrue
                      else
                          Flip.Expect.isFalse)
                      "If cached version is higher \
    ...
    testList
        ...
        [ let testArgs args () =
          ...
          testList
              "Commands"
              (commands
               |> Array.map (fun (cmdName, cmdValue) -> testCase cmdName <| testArgs [| cmdValue |])
               |> Array.toList)
          ...
          testList
              ...
              (flagsModules["Common"]
               |> Map.toList
               |> List.map (fun (flagName, flagValue) -> testCase flagName <| testArgs [| flagValue |]))

          testList
              "Common Flags needing values fail without"
          ...
          testList
              "Common Flags needing values succeed with"
              (flagArgsModules["Common"]
               |> Map.toList
               |> List.map (fun (flagName, flagValue) ->
                   testCase flagName
                   <| fun _ ->
                       try
                           Cli.parser.Parse([| flagValue; "somevalue" |]) |> Ok
                       with e ->
                           Error e
