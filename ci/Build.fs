...
Target.create Ops.checkChangeLogGen <| fun _ ->
    let fableElectronChanged =
        Information.getCurrentSHA1 Root.``.``
        |> FileStatus.getChangedFilesInWorkingCopy Root.``.``
        |> Seq.tryFind (snd >> fun file -> file.EndsWith("Fable.Electron/Program.fs") || file.EndsWith("Fable.Electron\\Program.fs"))
        |> function
            | Some _ -> true
            | _ -> false
    // Terminate early
    ...
    let currentVersion =
        Changelog.Changelog.findLastVersion file
        |> SemVer.parse
    let releaseVersion =
        match _release with
        | None ->
            failwith "Cannot run 'changelog-gen' without first downloading and running the api generator"
        | Some { tagName = rawTag } ->
    ...
    let newVersion =
        ...
        | { Patch = releasePatch }, { Patch = currentPatch } when
            ...
            { releaseVersion with
                Patch = if currentPatch >= releasePatch
                        then currentPatch + 1u
                        else releasePatch }
        | _ when releaseVersion.Major > currentVersion.Major || releaseVersion.Minor > currentVersion.Minor ->
            releaseVersion
        | _ ->
            failwith $"Unexpected version collision where the current version is higher than the generated version:\n \
    ...
    | Some newVersion ->
        ...
        |> function
            | ChangelogGenResult.Error e ->
                printfn $"Failed to generate changelog for %A{newVersion}: %s{e}"
            | ChangelogGenResult.NewVersion version ->
                Root.``RELEASE_NOTES.md``
                |> Git.Staging.stageFile Root.``.``
                |> ignore
                Commit.exec Root.``.`` $"chore: release %s{version}"
                Branches.tag Root.``.`` version
            | ChangelogGenResult.NoVersionBump ->
                printfn $"Failed to generate changelog for %A{newVersion} because it was not determined to be a \
...
Target.create Ops.gitCommit <| fun _ ->
    files
    |> List.iter (Git.Staging.stageFile Root.``.`` >> ignore)
    if _fableElectronNewVersion.IsSome
